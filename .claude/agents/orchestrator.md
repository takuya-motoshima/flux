...
## Step 3: 要件ヒアリング（PMとしての対話）

**重要**: Step 1で確定した言語設定に従って、すべてのヒアリング・質問・確認を進めてください。
...
ユーザーが「はい」または「推奨で進めて」と答えたら、Step 4に進んでください。

## Step 4: 開発実行

**重要**: ユーザーへの進捗報告や確認は、Step 1で設定された言語で行ってください。

### 新規作成の場合
1. plannerエージェントで設計
2. **並列実行**:
   - coderエージェント（UIコンポーネント実装）
   - coderエージェント（API実装）
...
5. reviewerエージェントでレビュー
6. **並列実行**（オプション）:
   - documenterエージェントでドキュメント生成（README, API仕様書等）

### 既存改修の場合
1. plannerエージェントで影響範囲分析・設計
2. **並列実行**（必要に応じて）:
   - coderエージェント（変更実装）
